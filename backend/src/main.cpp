...
int main() {
  try {
    ...
    // e.g. https://hetansh2744.github.io
    ...
    // Register
    srv.Post("/auth/register",
             [&](const httplib::Request& req, httplib::Response& res) {
               json body;
               if (!parseJsonBody(req, body)) {
                 return jsonError(res, 400, "BAD_JSON", "Invalid JSON",
                                  corsOrigin);
               }
               ...
               if (name.empty() || email.empty() || password.size() < 6) {
                 return jsonError(res, 400, "VALIDATION_ERROR",
                                  "name, email, password(>=6) required",
                                  corsOrigin);
               }

               std::string pwHash = Password::hash(password);

               const char* params[3] = {name.c_str(), email.c_str(),
                                        pwHash.c_str()};
               ...
               std::string token =
                   Jwt::signUser(userId, jwtSecret, 60 * 60 * 24);
    ...
    srv.Post("/auth/login",
             [&](const httplib::Request& req, httplib::Response& res) {
               ...
               if (email.empty() || password.empty()) {
                 return jsonError(res, 400, "VALIDATION_ERROR",
                                  "email and password required", corsOrigin);
               }

               const char* params[1] = {email.c_str()};
               PGresult* r = PQexecParams(
                   db.conn(),
                   "SELECT id, password_hash FROM users WHERE email=$1", 1,
               ...
               if (!r || PQresultStatus(r) != PGRES_TUPLES_OK ||
                 ...
                 return jsonError(res, 401, "INVALID_CREDENTIALS",
                                  "Invalid email or password", corsOrigin);
               }
    ...
    srv.Post("/transactions",
             [&](const httplib::Request& req, httplib::Response& res) {
               ...
               if ((type != "INCOME" && type != "EXPENSE") || amount <= 0 ||
                   date.empty() || category.empty() || title.empty()) {
                 return jsonError(
               ...
               PGresult* r = PQexecParams(
                   db.conn(),
                   "INSERT INTO transactions(user_id,type,amount,currency,tx_date,"
                   "category,title,note) "
               ...
               if (!r || PQresultStatus(r) != PGRES_TUPLES_OK) {
                 clearRes(r);
                 return jsonError(res, 500, "DB_ERROR",
                                  "Could not create transaction", corsOrigin);
               }
    ...
    srv.Get("/transactions",
            [&](const httplib::Request& req, httplib::Response& res) {
              ...
              if (!r || PQresultStatus(r) != PGRES_TUPLES_OK) {
                clearRes(r);
                return jsonError(res, 500, "DB_ERROR",
                                 "Could not fetch transactions", corsOrigin);
              }
    ...
    srv.Get("/summary",
            [&](const httplib::Request& req, httplib::Response& res) {
              ...
              if (!r || PQresultStatus(r) != PGRES_TUPLES_OK) {
                clearRes(r);
                return jsonError(res, 500, "DB_ERROR", "Could not fetch summary",
                                 corsOrigin);
              }
    ...
    // ✅ UPDATE transaction (must belong to user)
    ...
    srv.Put(R"(/transactions/(\d+))",
            [&](const httplib::Request& req, httplib::Response& res) {
              ...
              if (req.matches.size() < 2) {
                return jsonError(res, 400, "BAD_REQUEST", "Missing id",
                                 corsOrigin);
              }

              long txId = 0;
              if (!parseLong(req.matches[1], txId) || txId <= 0) {
                return jsonError(res, 400, "BAD_REQUEST", "Invalid id",
                                 corsOrigin);
              }
              ...
              const char* params[9] = {
                  type.c_str(), amtStr.c_str(), currency.c_str(), date.c_str(),
                  category.c_str(), title.c_str(), note.c_str(),
                  userStr.c_str(), txStr.c_str(),
              };

              PGresult* r = PQexecParams(
                  db.conn(),
                  "UPDATE transactions "
                  "SET type=$1, amount=$2, currency=$3, tx_date=$4, category=$5, title=$6, note=$7 "
              ...
              if (!r || PQresultStatus(r) != PGRES_COMMAND_OK) {
                clearRes(r);
                return jsonError(res, 500, "DB_ERROR",
                                 "Could not update transaction", corsOrigin);
              }

              const char* affected = PQcmdTuples(r);
              int updated = affected ? std::atoi(affected) : 0;
              clearRes(r);

              if (updated == 0) {
                return jsonError(res, 404, "NOT_FOUND",
                                 "Transaction not found", corsOrigin);
              }

              jsonOk(res, {{"ok", true}}, corsOrigin);
            });

    // ✅ DELETE transaction (must belong to user)
    ...
    srv.Delete(R"(/transactions/(\d+))",
               [&](const httplib::Request& req, httplib::Response& res) {
                 ...
                 const char* params[2] = {userStr.c_str(), txStr.c_str()};
                 ...
                 if (!r || PQresultStatus(r) != PGRES_COMMAND_OK) {
                   clearRes(r);
                   return jsonError(res, 500, "DB_ERROR",
                                    "Could not delete transaction", corsOrigin);
                 }
