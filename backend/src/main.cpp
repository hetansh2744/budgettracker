...
int main() {
  try {
    ...
    // For quick testing: "*" (but set to your frontend URL in production)
    const std::string corsOrigin = Env::get("CORS_ORIGIN", "*");

    // Connect DB + migrations
    Db db(dbUrl);
    ...
    // ✅ Homepage (so opening Render URL isn't "not found")
    srv.Get("/", [&](const httplib::Request&, httplib::Response& res) {
      addCors(res, corsOrigin);
      res.set_content(
        "<!doctype html><html><head><meta charset='utf-8'/>"
        "<meta name='viewport' content='width=device-width,initial-scale=1'/>"
        "<title>FlowFund API</title></head>"
        "<body style='font-family:system-ui;padding:24px'>"
        "<h1>FlowFund API is running ✅</h1>"
    ...
    srv.Get("/summary", [&](const httplib::Request& req, httplib::Response& res) {
      ...
      jsonOk(res, {{"income", income}, {"expense", expense}, {"balance", income - expense}}, corsOrigin);
    });
